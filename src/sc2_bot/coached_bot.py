...
class CoachedTerranBot(BotAI):
    ...
    def __init__(self, strategy: Optional[StrategyConfig] = None, worker_id: int = 1):
        ...
        # Critical Health Preservation & Degradation Tracking
        self.critical_hp_tracked: dict[int, float] = {}  # tag -> time first fell <= 35% HP
        self.critical_hp_events: int = 0
    ...
    async def _step_body(self, iteration: int):
        ...
        # Monitor Combat Unit HP degradation & preservation
        # (Exclude workers and buildings, focus on combat army)
        combat_filter = lambda x: (x.can_attack_ground or x.can_attack_air) and x.type_id not in {UnitTypeId.SCV, UnitTypeId.MULE}
        current_combat_units = self.units.filter(combat_filter)
        current_combat_tags = {u.tag for u in current_combat_units}

        for u in current_combat_units:
            hp_ratio = u.health / u.health_max
            if hp_ratio <= 0.35:
                if u.tag not in self.critical_hp_tracked:
                    self.critical_hp_tracked[u.tag] = self.time
                    self.critical_hp_events += 1
            elif hp_ratio >= 0.65 and u.tag in self.critical_hp_tracked:
                # Successfully pulled back, healed by Medivac or repaired!
                self.critical_hp_units_saved += 1
                del self.critical_hp_tracked[u.tag]

        # Check if any tracked critical unit died without recovery
        dead_tags = [t for t in self.critical_hp_tracked if t not in current_combat_tags]
        for dt in dead_tags:
            self.critical_hp_units_lost += 1
            del self.critical_hp_tracked[dt]
